  ...
  DocBlockrAtom = (function() {
    ...
    function DocBlockrAtom() {
      ...
      atom.commands.add('atom-workspace', 'docblockr:parse-tab', function(event) {
        ...
        // Parse Command
        if(self.validateRequest({preceding:true, preceding_regex:regex.parse})) {
          // console.log('Parse command');
          self.parseCommand(false);
        }

        // Indent Command
        else if(self.validateRequest({preceding:true, preceding_regex:regex.indent})) {
      ...
      atom.commands.add('atom-workspace', 'docblockr:parse-enter', function(event) {
        ...
        // Trim auto whitespace
        else if(self.validateRequest({preceding:true, preceding_regex:regex.trim_auto[0], following:true, following_regex:regex.trim_auto[1], scope:'comment.block'})) {
          // console.log('Auto Trim command');
          self.trimAutoWhitespaceCommand();
        }
        // Deindent command
        else if(self.validateRequest({preceding:true, preceding_regex:regex.deindent})) {
          // console.log('Deindent command');
          self.deindentCommand();
        }
        else if(self.validateRequest({preceding:true, preceding_regex:regex.snippet_1[0], following:true, following_regex:regex.snippet_1[1]})) {
          // console.log('Snippet-1 command');
          var editor = atom.workspace.getActiveTextEditor();
          self.write(editor, '\n$0\n ');
        }
        // Close block comment
        else if(self.validateRequest({preceding:true, preceding_regex:regex.close_block})) {
          // console.log('Snippet close block comment command');
          var editor = atom.workspace.getActiveTextEditor();
          self.write(editor, '\n$0\n */');
        }
        // extend line comments (// and #)
        else if((self.editor_settings.extend_double_slash == true) && (self.validateRequest({preceding:true, preceding_regex:regex.extend_line, scope:'comment.line'}))) {
        ...
        // Extend docblock by adding an asterix at start
        else if(self.validateRequest({preceding:true, preceding_regex:regex.extend, scope:'comment.block'})) {
      ...
      atom.commands.add('atom-workspace', 'docblockr:parse-inline', function(event) {
        ...
        if(self.validateRequest({preceding:true, preceding_regex:_regex}))
      ...
      atom.commands.add('atom-workspace', 'docblockr:join', function(event) {
        ...
        if(self.validateRequest({scope:'comment.block'}))
          self.joinCommand();
      });
     ...
     */
    DocBlockrAtom.prototype.validateRequest = function(options) {
      /**
    ...
    DocBlockrAtom.prototype.parseCommand = function(inline) {
      var editor = atom.workspace.getActiveTextEditor();
    ...
    DocBlockrAtom.prototype.trimAutoWhitespaceCommand = function() {
      /**
    ...
    DocBlockrAtom.prototype.indentCommand = function() {
      var editor = atom.workspace.getActiveTextEditor();
    ...
    DocBlockrAtom.prototype.joinCommand = function() {
      var editor = atom.workspace.getActiveTextEditor();
    ...
    DocBlockrAtom.prototype.decorateCommand = function() {
      var editor = atom.workspace.getActiveTextEditor();
    ...
    DocBlockrAtom.prototype.decorateMultilineCommand = function() {
      var editor = atom.workspace.getActiveTextEditor();
      var pos = editor.getCursorBufferPosition();
      var whitespace_re = /^(\s*)\/\*/;
    ...
    DocBlockrAtom.prototype.deindentCommand = function() {
      /*
    ...
    DocBlockrAtom.prototype.reparseCommand = function() {
    ...
    DocBlockrAtom.prototype.wrapLinesCommand = function() {
      /**
    ...
    DocBlockrAtom.prototype.getIndentSpaces = function(editor, line) {
    ...
    DocBlockrAtom.prototype.initialize = function(editor, inline) {
      ...
      settingsAlignTags = this.editor_settings.alignTags || 'deep';
    ...
    DocBlockrAtom.prototype.fillArray = function(len) {
      var a = [];
      var i = 0;
      while (i < len) {
        a[i] = 0;
    ...
    DocBlockrAtom.prototype.readLine = function(editor, point) {
    ...
    DocBlockrAtom.prototype.scopeRange = function(editor, point, scope_name) {
    ...
    DocBlockrAtom.prototype.getParser = function(editor) {
    ...
    DocBlockrAtom.prototype.generateSnippet = function(out, inline) {
      ...
      if(inline) {
        ...
        return (this.createSnippet(out) + (this.editor_settings.newline_after_block ? '\n' : ''));
    };

    DocBlockrAtom.prototype.substituteVariables = function(out) {
    ...
    DocBlockrAtom.prototype.alignTags = function(out) {
    ...
    DocBlockrAtom.prototype.fixTabStops = function(out) {
      var tab_index = this.counter();
    ...
    DocBlockrAtom.prototype.createSnippet = function(out) {
      var snippet = '';
